...
- [ ] 用户画像向量化和相似度计算
- [x] 采集结果去重和合并
- [x] JSON 采集配置(Collection)：关键词列表 + 抓取参数 + 匹配阈值，支持多套个性化配置
- [x] 夜间持续采集：到点自动停止 + 跨轮去重 + 周期落盘
- [x] 限流退避：GitHub/论文请求支持指数退避重试 + Retry-After/X-RateLimit-Reset
- [x] 逐轮加深采集：持续模式按轮次递增翻页偏移(page_offset)，多轮抓取更深内容
